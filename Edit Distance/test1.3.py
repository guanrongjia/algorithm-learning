...
''',
         'result': [
             ('T', '# ============== DELETEs =====================', '# ============== DELETEs ====================='),
             ('D', '# TODO: add docstrings', ''), ('T', "@app.route('/queue/<hostname>', methods=['DELETE'])",
                                                   "@app.route('/queue/<hostname>', methods=['DELETE'])"),
             ('T', 'def delete(hostname):', 'def delete(hostname):'),
             ('I', '', '    """Handle delete request from the given host"""'), ('T', '    try:', '    try:'),
             ('T', '        data = json.loads(request.get_data())', '        data = json.loads(request.get_data())'),
             ('S', "        mac_address = data['macAddress']", "        mac_address = data['mac_address']"),
             ('T', '    except:', '    except:'), ('T', "        abort(400, 'Missing or invalid user data')",
                                                   "        abort(400, 'Missing or invalid user data')"), (
             'S', '    status = queue.dequeue(hostname, macAddress)',
             '    status = queue.dequeue(hostname, mac_address)'),
             ('T', "    return ('', status)", "    return ('', status)"), ('T', '', ''), ('T', '', ''),
             ('T', "@app.route('/queue', methods=['DELETE'])", "@app.route('/queue', methods=['DELETE'])"),
             ('S', 'def empty_queue():', 'def clear_queue():'),
             ('I', '', '    """Clear the queue. Valid only if coming from tutor machine"""'), (
             'T', '    if request.remote_addr.upper() != TUTOR_MACHINE.upper():',
             '    if request.remote_addr.upper() != TUTOR_MACHINE.upper():'), (
             'S', '        abort(403, "Not authorised")',
             '        abort(403, "Only the tutor machine can clear the queue")'), ('T', '    else:', '    else:'),
             ('T', '        queue.clear_queue()', '        queue.clear_queue()'), (
             'S', '        response = jsonify({"message": "Queue emptied"})',
             '        response = jsonify({"message": "Queue cleared"})'),
             ('T', '        response.status_code = 204', '        response.status_code = 204'),
             ('T', '        return response', '        return response')]},
